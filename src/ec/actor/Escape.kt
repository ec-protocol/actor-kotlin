...
internal fun escape(pkg: ByteArray): ByteArray {
...
internal fun unescape(pkg: ByteArray) = contextAwareUnescape(pkg).first

internal fun contextAwareUnescape(pkg: ByteArray, startInEscape: Boolean = false): Pair<ByteArray, Boolean> {
    val ret = arrayListOf<Byte>()
    var escape = false
    pkg.forEach {
        when {
